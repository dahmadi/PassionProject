...
class WrappedPage extends React.Component {
	constructor(props) {
		...
		this.state = {
			...
			termLabels: ["Last month", "Last 6 months", "All time"],
			colors: [
	...
	componentDidMount() {
		...
		this.setState({
			albumOffset: Math.floor((mainPanelWidth - 126 - 24) / 5),
			albumContainerOffset: mainPanelWidth,
		});
	...
	async getTopItems(item, term) {
		return superagent
			.get(`https://api.spotify.com/v1/me/top/${item}`)
			...
			<div>
				<div className="d-md-none d-lg-none d-xl-none d-xxl-none d-lg-block d-xl-block">
							...
							<div style={styles.mobile_container} className="col-sm">
								...
								>
									<div id="main-panel" style={styles.main_card}>
										<div
											className="parent perspective"
											style={{ marginLeft: this.state.albumContainerOffset }}
										>
											{data.tracks.slice(0, 5).map((track, i) => {
												...
												return (
													<img
														...
														alt={i}
														id="track"
														style={{ right: offset, zIndex: index }}
														src={track.album.images[1].url}
														height="190"
														width="190"
														className="square-img child"
													/>
												);
											})}
										</div>
										<div className="container-sm">
											...
											<div className="row" style={{ marginTop: 20 }}>
												<h6 style={{ ...styles.topHeading, textAlign: "center" }}>
													TOP GENRES
												</h6>
												<div className="d-flex flex-column">
													{Array.from(data.genres.keys())
														.slice(0, 5)
														.map((genre, i) => {
															return (
																<p
																	key={i}
																	className="d-sm-block text-truncate"
																	style={{ ...styles.textItem, ...styles.genreText }}
																>
																	<b>{i + 1}</b>&nbsp;&nbsp;{genre}
																</p>
															);
										...
										>
											<p style={{ marginLeft: 20 }}>
												<small style={{ color: "#a6a6a6", fontSize: 10 }}>
													Swipe right to add to playlist &#8594;
												</small>
											</p>
										</div>
									</div>
									...
									<div id="main" style={styles.button_panel}>
										...
										>
											<button id="playlistBtn" onClick={this.addToPlaylist} className="login-btn">
												Add to playlist
											</button>
										</div>
									</div>
					...
					<div
						...
						className="fixed-bottom share-footer d-flex flex-row justify-content-between align-items-center"
					>
						<div className="d-flex flex-column">
							<p style={styles.footerTag}>#WRAPPEDWHENEVER</p>
						</div>
						<a href="#" onClick={this.updateTermCount} className="term-select" style={{ fontSize: 14 }}>
							{this.state.data[this.state.termCount % 3].label}
						</a>
					</div>
					<Toaster />
				</div>
